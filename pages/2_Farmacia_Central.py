...
st.set_page_config(page_title="Farmácia Central", page_icon="icone-unimed.png", layout="wide")
...
if df_farm is not None:
    ...
    with st.sidebar.expander("ℹ️ Legenda dos Filtros"):
        st.markdown("""
        <div style="height: 200px; overflow-y: auto; padding-right: 5px;">
            ...
            <ul style="margin-bottom: 0;">
                <li><strong>A:</strong> 80% do prejuízo.</li>
                <li><strong>B:</strong> 15% do prejuízo.</li>
                <li><strong>C:</strong> 5% do prejuízo.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.metric("💰 Desperdício Filtrado", valor_farm_fmt)
    with col2:
        qtd_total = df_filtrado['qt_estoque'].sum()
        st.metric("💊 Quantidade Total Desperdiçada", f"{qtd_total:,.0f}".replace(",", "."))
    with col3:
        st.metric("⚠️ Causa Principal", df_filtrado['ds_operacao'].mode()[0] if not df_filtrado.empty else "N/A")

    # Diagnóstico Estratégico
    st.markdown('<span style="font-size:2rem; font-weight:600">📊 Diagnóstico Estratégico</span>', unsafe_allow_html=True)
    
    st.markdown("""
    <style>
    /* Estilo direto para gráficos Plotly */
    .stPlotlyChart {
        background-color: #E8F5E9;
        border-radius: 12px;
        padding: 20px;
    ...
    # Gráfico de Pizza (Perfil)
    fig_perfil = px.pie(
        df_filtrado, values='vl_estoque', names='perfil_produto',
        title="<b>Onde está concentrado o dinheiro?</b>",
    ...
    # Lista de Ataque (Produtos Críticos)
    st.subheader("📋 Lista de Ataque (Produtos Críticos)")
    ...
    st.subheader("📈 Investigação Temporal: Onde o problema começou?")
    ...
    if df_full is not None:
        df_farm_full = df_full[df_full['ds_centro_custo'] == 'FARMÁCIA CENTRAL'].copy()
        ...
        st.warning("Base completa não disponível para análise temporal.")
    
    csv = df_tabela.to_csv(index=False).encode('utf-8')
    st.download_button("📥 Baixar CSV", data=csv, file_name="farmacia_lista_ataque.csv", mime="text/csv")

else:
    st.error("Arquivo 'df_dashboard_farmacia.csv' não encontrado.")